...
class HistoryStore:
        ...
        with self._connection() as connection:
            connection.execute(
                ...
                INSERT INTO report_history (
                    product_session_id, filename, created_at, overall_result,
    ...
    def search_reports(
        ...
        """Search summaries by filename text and/or an overall result."""
        limit = _valid_limit(limit)
        clauses: list[str] = []
        parameters: list[Any] = []
        if filename_query:
            clauses.append("filename LIKE ? COLLATE NOCASE")
            parameters.append(f"%{filename_query}%")
        if overall_result:
